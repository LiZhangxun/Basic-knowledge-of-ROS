...
*
*/
//对激光雷达源码进行了修改,直接将read_value 中的distance信息发布,而不采用scan_msg
#include "ros/ros.h"
...
void publish_scan(ros::Publisher *pub,
                  ...
                  std::string frame_id)
{
    sensor_msgs::LaserScan scan_msg;//实例化激光scan_msg数据
    delta_lidar::Laser_distance distance_msg;//实例化激光距离数据
    distance_msg.distance=0.0;//赋初值
    ...
    scan_msg.range_min = 0.15;//定义测距的最小值为0.15m
    scan_msg.range_max = 5.0;//定义测距的范围最大值为5m
    ...
    for (size_t i = 0; i < node_count; i++)
    {
        ...
        if (read_value < scan_msg.range_min || read_value > scan_msg.range_max)
            scan_msg.ranges[360- 1- current_angle] = std::numeric_limits<float>::infinity();//发布float数据极限值(inf)
        else
        {
            scan_msg.ranges[360 -1- current_angle] = read_value;
            distance_msg.distance=read_value; //赋值给自定义数据
            printf("Receive Lidar distance %f!\n", read_value);//发布激光雷达真实的距离信息(直接雷达测出来的,还未发布scan_msg)
        }
    ...
    pub->publish(scan_msg);//发布激光雷达数据
    distance_pub->publish(distance_msg);//发布自定义数据类型
}
...
//    ros::Publisher scan_pub = nh.advertise<sensor_msgs::LaserScan>(lidar_scan, 1000);
    ...
    ros::Publisher distance_pub = nh.advertise<delta_lidar::Laser_distance>("distance_pub", 1000);//创建距离信息发布者
    ...
    while (ros::ok())
    {
        ...
        switch(result)
        {
            ...
            case LIDAR_GRAB_SUCESS:
            {
                ...
                publish_scan(&scan_pub,&distance_pub, &send_lidar_scan_data[0], lidar_scan_size,
                         start_scan_time, scan_duration,
                         angle_min, angle_max,
                         frame_id);
